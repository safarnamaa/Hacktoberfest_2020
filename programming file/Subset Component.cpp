...
{
if(x!=comp[x])comp[x]=fcomp(comp[x],comp,size);
return comp[x];
}
...
a=fcomp(a,comp,size);
b=fcomp(b,comp,size);
if(size[a]>size[b])comp[b]=a;
else if(size[a]<size[b])comp[a]=b;
else
{
comp[a]=b;
size[b]++;
...
for(int i=0;i<64;++i)create(i,comp,size);
::k=k;
::n=n;
    info.resize(n);
